 ...
 * @since     1.0
 */
class LegacyLogin_WellspringHelper extends LegacyLogin_AuthenticationHelper
{
	...
	public static function login($loginName = null, $password = null, $rememberMe = false)
	{
		...
		LegacyLoginPlugin::log("Attempting to login {$loginName} as a Wellspring legacy user...");
		...
		if ($success)
		{
			LegacyLoginPlugin::log("{$loginName} logged in as a Wellspring legacy user.", LogLevel::Info, true);
		}
		else
		{
			LegacyLoginPlugin::log("{$loginName} was authenticated as a Wellspring legacy user, but could not be logged in.", LogLevel::Error);
		}
		return $success;
	...
	public static function getMatchedUser($loginName = null)
	{
		...
		$matchedUserRecord = LegacyLogin_MatchedUserRecord::model()->find(array(
			...
			'params' => array(
				...
				':legacyUserType' => LegacyLoginPlugin::WellspringLegacyUserType
			),
		));
	...
	private static function _getLegacyUserData($loginName = null)
	{
		...
		// See if matched user data exists
		$legacyUserRecord = LegacyLogin_WellspringUserDataRecord::model()->find(array(
			'condition' => 'user_email=:loginName OR user_id=:loginName',
			'params' => array(
				':loginName' => $loginName,
			),
		));
	...
	public static function authenticate($legacyData = null, $password = null)
	{
		...
		if ($legacyData->user_pass === sha1($password))
		{
			return true;
		}
	...
	public static function makeMatchedUser($legacyData = null, $password = null)
	{
		...
		if (empty($craftUser))
		{
			...
			$craftUser->username  = $legacyData->user_email;
			$craftUser->firstName = $legacyData->user_name;
		...
		if ($success)
		{
			...
			$matchedUser->legacyUserType = LegacyLoginPlugin::WellspringLegacyUserType;
			$matchedUser->legacyRecordId = $legacyData->id;
			$matchedUser->legacyUserId = $legacyData->user_id;
